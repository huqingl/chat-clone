...
const AnswerSection = ({ storedValues,canInput }) => {
  //随着内容增加，自动滚动到底部
      ...
      <div className="answer-container overflow-hidden h-full">
        <div className="answer-wrapper h-full overflow-y-auto">
          <div className="prompt-wrapper border-gray-300 bg-gray-100">
            <div className="prompt w-3/5 mt-0 mx-auto flex pb-4  py-5">
              <div className="pr-5">
                <IconFont
          ...
          <div className="answer-content-box " >
            {storedValues.map((value, index) => {
              ...
              return (
                <div className={`${value.role === "assistant" ? "bg-gray-100" : ""
                  } answer-section-wrapper w-full`} style={{ borderBottom: '1px solid #eee' }} key={index}>
                  <div className="answer-section w-3/5 mt-0 mx-auto text-base " >
                    <div className="answer flex py-5">
                      <div className="logo pr-5">
                        <IconFont type={person} style={logoColor} />
                      </div>
                      <div className="answer-content text-base pt-0.5" style={{ whiteSpace: 'pre-line' }}>
                        <HighlightedResponse response={value.content} />
                        <span className={value.role === "assistant" && index === storedValues.length -1 && canInput ? "block cursor":"hidden" }></span>
                      </div>
                    </div>
                  </div>
                </div>
